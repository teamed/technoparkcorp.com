...
class Model_Navigation {
    ...
    public static function populateNavigation(&$container, $activePage) {
        self::_addMenuPages($container, $activePage);
    }
     ...
     * @return void
     */
    protected static function _addMenuPages($container, $activePage, $path = CONTENT_PATH, $prefix = '') {
        ...
        foreach (glob($path . '/*.xml') as $file) {
            ...
            if (is_dir($path . '/' . $label))
                self::_addMenuPages($page, $activePage, $path . '/' . $label, $fullLabel);
        }
