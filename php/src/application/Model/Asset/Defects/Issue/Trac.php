...
class Model_Asset_Defects_Issue_Trac extends Model_Asset_Defects_Issue_Abstract
{
    ...
    public function askOnce($code, $text, $lag = null) 
    {
        ...
        if (!is_null($lag) && $lastDate->isEarlier(Zend_Date::now()->subDay($lag))) {
            logg(
    ...
    public function exists() 
    {
        ...
        $ids = $this->_tracker->retrieveBy(
            array('code'=>Model_Pages_Encoder::encode($this->code))
        );
        ...
        switch ($name) {
            case 'code':
                if ($value) {
                    ...
                    } catch (Model_Pages_Encoder_InvalidSalt $e) {
                        $name = null;
                    }
                } else {
                    $name = null;
