...
% watermark 
<? if (!$this->sheets['Offer']->hasPrice()): ?>
    \chead{
        \begin{textblock}{16}(0,0)
            \tikz{
                \node [rotate=45, scale=20, text=tpcGrey!3, font=\ttfamily\bfseries] at (0,0) {draft};
            }
        \end{textblock}
    }
<? endif; ?>
...
\vspace*{3cm}
\parbox{\linewidth}{\linespread{1}\selectfont\small
TechnoPark Corp. accepts no liability for the content of this document, 
or for the consequences of any actions taken on the basis 
of the information provided. This document is not a contract or a binding agreement
of any kind, but merely a sales brochure.\linebreak
}
\clearpage
...
<? foreach ($this->sheets as $sheet): ?> 
    <? if (!Sheet_Abstract::isTemplateExists($sheet->getProposalFile())) continue; ?>
    <? $this->assign('sheet', $sheet) ?>
    <?=$this->render($sheet->getProposalFile())?>
    <? if (Sheet_Abstract::isTemplateExists($sheet->getTemplateFile())): ?>
...
\appendix
<? foreach ($this->sheets as $sheet): ?>
    <? if (!Sheet_Abstract::isTemplateExists($sheet->getTemplateFile())) continue; ?>
